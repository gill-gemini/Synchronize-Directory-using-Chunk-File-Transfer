...
#define UDPHDRSIZE sizeof(struct sniff_udp)
//do we need to add a udp statment also?
...
void ParseUDPPacket(const u_char *packet,char *srcHost, char *dstHost);
...
*/
/*????????????????????????????????this is TCP check sum we need it ??*/
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
  ...
  printf("sourceaddress%s\t->\t", srcHost);
  printf("destinationaddress%s\t", dstHost);
  ...
  switch(ip->ip_p) {
    ...
    case IPPROTO_UDP:
      printf("UDP\t");
      ParseUDPPacket((u_char *)ip,srcHost,dstHost);
      break;
    case IPPROTO_ICMP:
      printf("ICMP\t");
      break;
...
void ParseUDPPacket(const u_char *packet,char *srcHost, char* dstHost)
{
    ...
    char *temp=NULL;
    char NOF[100] = { 0 };
    char chunkrange[50]={0};
    char sourcehost[INET_ADDRSTRLEN];
    char desthost[INET_ADDRSTRLEN];
    ip = (struct sniff_ip*)(packet);
    udp = (struct sniff_udp*)(packet + IPHDRSIZE);
        /*check how to compute the size of the udp header??*/
    ipHdrSize = IP_HL(ip)*4;
    size_udp = ntohs(udp->len); // the size of whole UDP packet (header+data)
      if (size_udp < UDPHDRSIZE)
      {
        printf(" * Invalid UDP header length: %u bytes\n", size_udp);
        return;
      }

//    printf("*******  size_ip:%d, size_udp:%d ************\n", size_ip, size_udp);


  	srcport = ntohs(udp->sport);
   	dstport = ntohs(udp->dport);

    //Printing Source Port and Destination Port
	printf("sourceport%d\t->\t\n", srcport);
	printf("destinationport %d\t\n", dstport);
      ...
      if(payload==NULL)
	{
	printf("above operation failed");
	}
    /* compute udp payload (segment) size */
    size_payload = ntohs(ip->ip_len) - (ipHdrSize + UDPHDRSIZE);

    printf("*******  size_ip:%d, size_udp:%d payloadOffset:%d************\n", ipHdrSize, size_udp, size_payload);

    // now we look for the "SessionID:" and "CSeq:"

    const char * SESSION_ID_TOKEN = "SessionID: ";
    const char * DELIVER_TOKEN = "DELIVER";

	//size_payload=1500;      
	char lData[MAX_BUF_SIZE];
	memset(lData, 0, sizeof(lData));

	// copy the payload data to lData;
	memcpy(lData, payload, size_payload);

	// see if we can find sessionid in the payload data
	if ((NULL == strstr(lData, SESSION_ID_TOKEN))
			|| (NULL == strstr(lData, DELIVER_TOKEN)))
	{
		// this is not the one we're interested in
		return;
	}


	printf("Found an interesting packet\n!");
    printf("*********Payload Printed%s\n",lData);
	char sessionid[500] ;
	memset(sessionid, 0, sizeof(sessionid));

	ushort sessionIdStartPos = (strstr(lData, SESSION_ID_TOKEN) - lData) + strlen(SESSION_ID_TOKEN);
	const char * endtoken = "\x0d\x0a";
	char * endTokenPtr = strstr(lData+sessionIdStartPos, endtoken);



	if (NULL == endTokenPtr)
	{
		printf("man, I cann't find the 0d0a pattern!\n");
		return ;
	}
	ushort sessionIdEndPos = endTokenPtr - lData;

	int sessionIdLength = sessionIdEndPos - sessionIdStartPos;
	printf("sessionIdLength = %d\n", sessionIdLength);
	memcpy(sessionid, lData+sessionIdStartPos, sessionIdLength);
	printf("sessionid is %s \n", sessionid);


	printf("sourceaddress%s\t->\t", inet_ntoa(ip->ip_src));
	printf("destinationaddress%s\t", inet_ntoa(ip->ip_dst));

	printf("Find Session Id%s\n",sessionid);
	...
	sprintf(buffer, "THANKS filename\r\n"
			"ChunkRange: 1-62\r\n"
			"NumberOfChunks: 62\r\n"
			"CSeq: 2 THANKS\r\n"
			"SessionID: %s\r\n"
...
*/
  ...
  //iph->ip_p = IPPROTO_UDP; /* IPPROTO_TCP or IPPROTO_UDP */
